...
def generateHeader(asset):
	f = open("src/generated/assets/" + asset.name.lower() + ".h", "w")
	print("/*", file=f)
	print(" * Copyright (C) 2016 necropotame (necropotame@gmail.com)", file=f)
	print(" * ", file=f)
	print(" * This file is part of TeeUniverse.", file=f)
	print(" * ", file=f)
	print(" * TeeUniverse is free software: you can redistribute it and/or  modify", file=f)
	print(" * it under the terms of the GNU Affero General Public License, version 3,", file=f)
	print(" * as published by the Free Software Foundation.", file=f)
	print(" *", file=f)
	print(" * TeeUniverse is distributed in the hope that it will be useful,", file=f)
	...
	print(" * GNU Affero General Public License for more details.", file=f)
	print(" *", file=f)
	print(" * You should have received a copy of the GNU Affero General Public License", file=f)
	print(" * along with TeeUniverse.  If not, see <http://www.gnu.org/licenses/>.", file=f)
	print(" */", file=f)
	print("", file=f)
	print("/*", file=f)
	...
	print(" *", file=f)
	print(" * Please use the script \"scripts/assets/assets.py\" to regenerate it.", file=f)
	print(" *", file=f)
	print(" * Why this file is generated by a script?", file=f)
	print(" * Because there is more than 10 files that follow the same format.", file=f)
	print(" * In addition, each time a new member is added, enums, getter, setters,", file=f)
	print(" * copy/transfert functions and storage structure must be updated.", file=f)
	print(" * It's too much to avoid mistakes.", file=f)
	print(" */", file=f)
	print("", file=f)
	print("#ifndef __CLIENT_ASSETS_" + asset.name.upper() + "__", file=f)
	print("#define __CLIENT_ASSETS_" + asset.name.upper() + "__", file=f)
	print("", file=f)
	print("#include <shared/assets/asset.h>", file=f)
	
	for h in list(set(asset.headers())):
		print("#include <"+h+">", file=f)
	
	print("", file=f)
	print("\n".join(asset.generateClassDefinition()), file=f)
	print("", file=f)
...
def generateImpl(asset):
	f = open("src/generated/assets/" + asset.name.lower() + ".cpp", "w")
	print("/*", file=f)
	print(" * Copyright (C) 2016 necropotame (necropotame@gmail.com)", file=f)
	print(" * ", file=f)
	print(" * This file is part of TeeUniverse.", file=f)
	...
	print(" */", file=f)
	print("", file=f)
	print("#include <generated/assets/"+asset.name.lower()+".h>", file=f)
	print("#include <shared/assets/assetssaveloadcontext.h>", file=f)
	print("#include <shared/archivefile.h>", file=f)
	print("", file=f)
...
f = open("src/generated/assets/allassets.h", "w")
print("/*", file=f)
print(" * Copyright (C) 2016 necropotame (necropotame@gmail.com)", file=f)
print(" * ", file=f)
print(" * This file is part of TeeUniverse.", file=f)
...
print(" */", file=f)
print("", file=f)
print("#ifndef __GENERATED_ASSETS_ALLASSETS__", file=f)
print("#define __GENERATED_ASSETS_ALLASSETS__", file=f)
print("", file=f)
...
f = open("src/generated/assets/assetsmacro.h", "w")
print("/*", file=f)
print(" * Copyright (C) 2016 necropotame (necropotame@gmail.com)", file=f)
print(" * ", file=f)
print(" * This file is part of TeeUniverse.", file=f)
...
print("", file=f)
for asset in assetsList:
	print("MACRO_ASSETTYPE("+asset.name+")", file=f)
print("", file=f)
